...
@pytest.mark.asyncio
async def test_acknowledge_event(httpx_mock): # type: ignore
    ...
    async with httpx.AsyncClient() as client:
        ...
        await api.acknowledge_event()


@pytest.mark.asyncio
async def test_enable_absence(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/absence/enable'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.enable_absence()


@pytest.mark.asyncio
async def test_disable_absence(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/absence/disable'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.disable_absence()


@pytest.mark.asyncio
async def test_pause_leakage_protection(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/leakageprotection/pause?minutes=10'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.pause_leakage_protection(10)


@pytest.mark.asyncio
async def test_unpause_leakage_protection(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/leakageprotection/unpause'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.unpause_leakage_protection()


@pytest.mark.asyncio
async def test_start_self_test(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/selftest'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.start_self_test()


@pytest.mark.asyncio
async def test_get_measurements(httpx_mock): # type: ignore
    ...
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        resp = await api.get_measurements()
...
@pytest.mark.asyncio
async def test_start_micro_leakage_measurement(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/mlmeasurement/start'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.start_micro_leakage_measurement()


@pytest.mark.asyncio
async def test_get_daily_statistics(httpx_mock): # type: ignore
...
@pytest.mark.asyncio
async def test_get_todays_consumption(httpx_mock): # type: ignore
    ...
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        resp = await api.get_todays_consumption()
        assert resp == 123.4


@pytest.mark.asyncio
async def test_get_total_consumption(httpx_mock): # type: ignore
    ...
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        resp = await api.get_total_consumption()
        assert resp == 12345678.9


@pytest.mark.asyncio
async def test_open_water_supply(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/watersupply/open'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.open_water_supply()


@pytest.mark.asyncio
async def test_close_water_supply(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/watersupply/close'
    httpx_mock.add_response(url=URL) # type: ignore
    async with httpx.AsyncClient() as client:
        auth = AsyncAuth(client, "")
        api = AsyncApiClient(auth)
        await api.close_water_supply()


@pytest.mark.asyncio
async def test_get_state_1(httpx_mock): # type: ignore
...
@pytest.mark.asyncio
async def test_get_state_2(httpx_mock): # type: ignore
    URL = 'https://appapi.watercryst.com/v1/state?locale=de&format=plain'
    JSON: dict[str, Any] = {
        "online": True,
        "mode": {
